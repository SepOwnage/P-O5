...
void STS_message1_master(message_ctx *message, STS_ctx *STS_ctx_master){
    ...
    //generate c
    memcpy(STS_ctx_master->base, GENERATOR, PRIME_SIZE);

    //send c
    message->tag = 0x00;
    message->length = PRIME_SIZE;
    memcpy(message->data, STS_ctx_master->base, PRIME_SIZE);
}
...
void STS_message1_slave(message_ctx *message, STS_ctx *STS_ctx_slave, const RSA_ctx *RSA_ctx_slave, const message_ctx *input){
    ...
    //read c
    memcpy(STS_ctx_slave->base, input->data, PRIME_SIZE);

    //generate b calculate c^b mod p
    do{isaac_rand(STS_ctx_slave->private, DH_PARAMETERS*8);
    ...
    message->tag = 0x01;
    message->length = PRIME_SIZE;
    memcpy(message->data, STS_ctx_slave->check, PRIME_SIZE);
}
...
void STS_message2_master(message_ctx *message, ENC_ctx *ENC_ctx_master, STS_ctx *STS_ctx_master, const RSA_ctx *RSA_ctx_master, const message_ctx *input){
    ...
    hash(ENC_ctx_master->nonce, NONCE_LENGTH, STS_ctx_master->base, PRIME_SIZE);
    unsigned int * temporary = (unsigned int *) (ENC_ctx_master->nonce+4);
    *temporary = ENC_ctx_master->counter;

    //generate a and calculate c^a mod p
    do{isaac_rand(STS_ctx_master->private, DH_PARAMETERS*8);
    ...
    //read c^b mod p
    memcpy(STS_ctx_master->base, input->data, PRIME_SIZE);

    //send c^a mod p
    message->tag = 0x02;
    message->length = PRIME_SIZE;
    memcpy(message->data, STS_ctx_master->check, PRIME_SIZE);
}
...
void STS_message2_slave(message_ctx *message, ENC_ctx *ENC_ctx_slave, STS_ctx *STS_ctx_slave, const RSA_ctx *RSA_ctx_slave, const message_ctx *input){
    ...
    unsigned char KEY[PRIME_SIZE];
    unsigned char TEMP[2*PRIME_SIZE];

    //calculate hash(c) and set counter part 0
    hash(ENC_ctx_slave->nonce, NONCE_LENGTH, STS_ctx_slave->base, PRIME_SIZE);
    unsigned int * temporary = (unsigned int *) (ENC_ctx_slave->nonce+4);
    *temporary = ENC_ctx_slave->counter;
    ...
    hash(KEY, AES_KEY_SIZE, KEY, PRIME_SIZE);
    aes_set_encrypt_key(&(ENC_ctx_slave->key),KEY, AES_KEY_SIZE*8);

    //compute hash(c^b|c^a)
    memcpy(TEMP, STS_ctx_slave->check, PRIME_SIZE);
    memcpy(TEMP+PRIME_SIZE, STS_ctx_slave->base, PRIME_SIZE);
    hash(KEY, MODULUS_SIZE_SLAVE, TEMP, 2*PRIME_SIZE);
    ...
    CCM_encrypt(message, ENC_ctx_slave, TEMP, MODULUS_SIZE_SLAVE);
    message->tag = 0x03;
}
...
void STS_message3_master(message_ctx *message, unsigned char *status, ENC_ctx *ENC_ctx_master, STS_ctx *STS_ctx_master, const RSA_ctx *RSA_ctx_master, const message_ctx *input){
    ...
    //compute hash(c^b|c^a)
    memcpy(TEMP, STS_ctx_master->base, PRIME_SIZE);
    memcpy(TEMP+PRIME_SIZE, STS_ctx_master->check, PRIME_SIZE);
    hash(TEMP, MODULUS_SIZE_SLAVE, TEMP, 2*PRIME_SIZE);
    mod_reduction(TEMP,TEMP, MODULUS_SLAVE, MODULUS_SIZE_SLAVE);

    if (memcmp(KEY,TEMP,MODULUS_SIZE_SLAVE)!=0)
    {
    ...
    memcpy(TEMP, STS_ctx_master->check, PRIME_SIZE);
    memcpy(TEMP+PRIME_SIZE, STS_ctx_master->base, PRIME_SIZE);
    hash(KEY, MODULUS_SIZE_MASTER, TEMP, 2*PRIME_SIZE);
    ...
    CCM_encrypt(message, ENC_ctx_master, TEMP, MODULUS_SIZE_MASTER);
    message->tag = 0x04;
...
void STS_message3_slave(message_ctx *message, ENC_ctx *ENC_ctx_slave, STS_ctx *STS_ctx_slave, const RSA_ctx *RSA_ctx_slave, const message_ctx *input){
   ...
   //compute hash(c^a|c^b)
    memcpy(TEMP, STS_ctx_slave->base, PRIME_SIZE);
    memcpy(TEMP+PRIME_SIZE, STS_ctx_slave->check, PRIME_SIZE);
    hash(TEMP, MODULUS_SIZE_MASTER, TEMP, 2*PRIME_SIZE);
    mod_reduction(TEMP,TEMP, MODULUS_MASTER, MODULUS_SIZE_MASTER);


    if (memcmp(KEY,TEMP,MODULUS_SIZE_MASTER)!=0)
    {
        message->tag = 0x05;
        message->length = 0;
    };
