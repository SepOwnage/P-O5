...
\title{H09M0A P\&D Embedded Systems and Multimedia \\ Speech codec project}
\author{Seppe Iven - r0370830 \\ Koen Goetschalckx - r0375967}
\begin{document} 
\maketitle
\section{Introduction}
This is a final report for the P\&D assignment on creating a speech codec. The goal of this project is to run an algorithm on a Digital Signal Processor that encodes, encrypts, decrypts and decodes speech in real-time. The encryption and decryption code is provided by another group. The codec implements subband filtering and adaptive differential quantization, with the goal of efficiently compressing a stereo audio signal.\\

This paper follows a certain order. First, the principles that are used in the codec are explained: the subband filtering and the adaptive differential quantization. Then, an implementation in MATLAB is discussed, which is optimized based on PESQ scores. The MATLAB code is then implemented in C, and the differences with the original MATLAB code will be discussed. The final part includes the porting onto and optimizing for a DSP, followed by the conclusions and a course evaluation.
...
While translating the MATLAB code to C (see below), it became clear that although the MATLAB code can use asymmetric filter banks, it is not implemented properly: a path with less filters or filters of smaller order is not extra delayed, which is needed to have the same delay as the longer path. This error is not fixed, since this implementation does not use an asymmetric structure anyway.

\section{C implementation on a PC}
Given the MATLAB model for the codec, the next task is to translate this code into an efficient implementation in C. This can later on be ported onto a Digital Signal Processor. Some of the key differences between the C and MATLAB code will now be discussed, followed by preparations for communication with a crypto group, and the performance enhancements that were made in the C implementation.
...
These are some things that specifically apply to us, and we will remember from this project:
\begin{itemize}
