...
\mytodo{include everything from ppt presentation}
...
\mytodo{see if there's anything useful in the matlab conclusion and use this to end the matlab part of this paper}

\section{C code}
\mytodo{maybe include this in matlab conclusion}
...
\mytodo{spreken over het feit dat we mono sample als 1 sample zien en andere groepen het als 2 dus eigenlijk zjin we 2x sneller}
After already speeding up the code by a factor of about 2, running the application with an input file of 2986 samples resulted in 21.8M cycles: 7.300 cycles / sample. The bottleneck was, as we suspected by analytical approach, convolve. This is because convolve contained the only nested loop, and worked with the biggest data types. \\
...
EN VOEG AFBEELDIGN TOE}\\
After improving these functions, convolve was once again the bottleneck. \mytodo{
Convolve:
Division as shift:  46\% on convolve
	 eliminates function call
Original:
Division for roundig to zero
New:
Complex division as shift for rounding behaviour+speed
EN VOEG AFBEELDING TOE}\\
...
After these major optimizations, the program needed 3.2M cycles for 2986 samples: \mytodo{cycles/sample?}
Some of the other adjustments, which did not net a large gain or were simply reveretd for clealiness, are: \mytodo{optimizations}

Another optimization that we have implemented is what we call 'cheatMono': We noticed that a lot of the input signals were mono instead of stereo, and thus a lot of computational power was being wasted. As a proof of concept, we wrote new functions 'quantizeMono' and 'dequantizeMono' which operated on the left channel and copied the results to the right channel. In the case of stereo input, the only change is one extra if-statement. This optimization results in a reduction of 16\% of the cycles of the whole program. The downside is that 2\% more of the available space is used. 
...
The functions of the DSP code are the same as the C code and will thus not be explained again. \mytodo{is dit al explained in het C gedeelte?}
...
\section{Conclusions}
\mytodo{Each team submits a final report as a PDF file. It contains an overview of the work and an evaluation of the course.
This report is not a day-to-day overview of your activities; it is more an executive overview. It has to integrate the content of the intermediate reports and also has to elaborate on the topics listed in the DSP final report format document (under Projects/DSP optimization).

The part about the course evaluation bundles the remarks and suggestions for improvement of the logistics as well as of the overall course planning.}
\section{Appendix}
