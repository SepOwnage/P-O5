...
void convolveFilter2Even(short *input,
	...
	unsigned short j; //bookkeepings
	long long result; //holds the temporary unscaled result
	unsigned short stop = inputL - 16; // the amount of samples that can be calculated
	short sample;
	short *samplepointer;
	short *endOfInputArray = input + inputL;
...
void analysis(short buffer[BUFFERSIZE],struct chunk *historyChunk){
	...
	//8 times convolve
	convolveFilter2Even(historyChunk->leftLowEven, historyChunk->position2,
	...
	convolveFilter2Odd(historyChunk->leftLowOdd, historyChunk->position2,
			BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, historyChunk->leftLowOdd,
			historyChunk->position2, BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, 15);
	convolveFilter3Even(historyChunk->leftHighEven, historyChunk->position2,
			BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, historyChunk->leftHighEven,
			historyChunk->position2, BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, 15);
	convolveFilter3Odd(historyChunk->leftHighOdd, historyChunk->position2,
			BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, historyChunk->leftHighOdd,
			historyChunk->position2, BUFFERSIZE_DIV8+LENGTH_FILTER2_HALF, 15);

	convolveFilter2Even(historyChunk->rightLowEven, historyChunk->position2,
			BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, historyChunk->rightLowEven,
			historyChunk->position2, BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, 15);
	convolveFilter2Odd(historyChunk->rightLowOdd, historyChunk->position2,
			BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, historyChunk->rightLowOdd,
			historyChunk->position2, BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, 15);
	convolveFilter3Even(historyChunk->rightHighEven, historyChunk->position2,
			BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, historyChunk->rightHighEven,
			historyChunk->position2, BUFFERSIZE_DIV8 + LENGTH_FILTER2_HALF, 15);
	convolveFilter3Odd(historyChunk->rightHighOdd, historyChunk->position2,
